...
//        configureInfo.scissor.extent = {width, height};
    ...
    void Pipeline::createShaderBindingTable() {
        ...
        VkDeviceSize sbtSize = m_genRegion.size + m_missRegion.size + m_hitRegion.size;
        m_shaderBindingTableBuffer = new Buffer(device, sbtSize, 1,
        ...
        auto getHandle = [&](uint32_t groupIndex) -> const uint8_t * {
            return shaderHandles.data() + groupIndex * handleSize;
        };
